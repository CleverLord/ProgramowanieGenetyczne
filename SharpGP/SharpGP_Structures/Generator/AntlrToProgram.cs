...
public class AntlrToProgram : SharpBaseVisitor<Node> {
	...
	public override Node VisitCondition(ConditionContext ctx) => new Condition(Visit(ctx.GetChild(0)), Visit(ctx.compareOp()), Visit(ctx.GetChild(2)));
	...
	public override Node VisitNestedExp(NestedExpContext ctx) => new NestedExpression(new List<Node>() {Visit(ctx.GetChild(1)), Visit(ctx.operand()), Visit(ctx.GetChild(3))});
	...
	public override Node VisitOperand(OperandContext ctx) => new Operator(ctx.GetChild(0).GetText());
	public override Node VisitCompareOp(CompareOpContext ctx) => new CompareOp(ctx.GetChild(0).GetText());
}
