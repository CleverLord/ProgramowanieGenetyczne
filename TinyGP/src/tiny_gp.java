...
public class tiny_gp {
    ...
    public String indivString="";
    int print_indiv(char[] buffer, int buffercounter) {
        int a1 = 0, a2;
        if (buffer[buffercounter] < FSET_START) {
            if (buffer[buffercounter] < varnumber) {
                System.out.print("X" + (buffer[buffercounter] + 1) + " ");
                indivString += "X" + (buffer[buffercounter] + 1) + " ";
            }
            else {
                System.out.print(x[buffer[buffercounter]]);
                indivString += x[buffer[buffercounter]];
            }
    ...
    void stats(double[] fitness, char[][] pop, int gen) {
        ...
        if(printStats) {
            ...
            try {
                Files.write(Path.of(resultPath + fname.replace(".dat","-result") + ".txt"),indivString.getBytes());
                //BufferedWriter writer = new BufferedWriter(new FileWriter(resultPath + fname + ".txt"));
                //writer.write(indivString);
                //writer.close();
            } catch (IOException e) {
    ...
    public tiny_gp(String fname, long s) {
        ...
        seed = s;
        this.fname=fname.substring(fname.lastIndexOf('\\'));
        if (seed >= 0)
            rd.setSeed(seed);
        setup_fitness(fname);
    ...
    String resultPath="Data/Results/Generated/Exp/";
    public static void main(String[] args){
        int s=-1;
        File folder = new File("Data/pythonData");
        var listOfFiles = Arrays.stream(folder.listFiles())
                .filter(File::isFile)
                .filter(f->f.getName().contains(".dat"))
                .filter(f->f.getName().contains("exGauss"))
                .map(File::toString).toList();
        listOfFiles.forEach(f->doTest(f,-1));
    }
    public static void doTest(String file,long s){
        System.out.println("Processing "+file+" ");
        System.out.println(DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm:ss").format(LocalDateTime.now()));
        long t1=System.currentTimeMillis();
        new tiny_gp(file, s).evolve();
        long t2 =System.currentTimeMillis();

        System.out.print("Time for processing: "+(t2-t1)+"ms\n");
    }
}